...
async function sendWithWallet(
  ...
  try {
    ...
    throw new Error(`Wallet could not sign this transaction: ${message}`);
  }
...
export async function connectBrowserWallet(): Promise<BrowserWallet> {
  ...
  // A cached provider.publicKey does not prove Phantom has an active signing
  // session. Always complete the provider handshake before the app reports a
  // wallet as connected; otherwise the first transaction fails in Phantom.
