...
const COMMIT_STEPS: Array<{ key: CommitmentProgress | "wallet"; label: string; detail: string }> = [
  ...
  { key: "delegating", label: "Private handoff", detail: "Your commitment account moves into the hosted TEE." },
...
const ORGANIZER_STEPS: Array<{ key: OrganizerProgress; label: string }> = [
  { key: "selecting-offer", label: "Select the best viable quote" },
  ...
  { key: "settling", label: "Pay supplier and unlock refunds" },
...
function DevnetWalletChecklist({ connected, usdcBalance }: { connected: boolean; usdcBalance: number | null }) {
  return <section className="getting-started devnet-checklist"><div><span className="app-kicker">DEVNET WALLET SETUP</span><h2>One Phantom wallet. Two free test assets.</h2><p>Keep Phantom on <strong>Solana Devnet</strong>. Devnet SOL pays transaction fees; devnet USDC is the test currency for the private-payment upgrade. Neither has real-world value.</p><div className="faucet-actions"><a className="faucet-link" href={SOLANA_DEVNET_FAUCET} target="_blank" rel="noreferrer">1. Get devnet SOL ↗</a><a className="faucet-link subtle" href={CIRCLE_DEVNET_FAUCET} target="_blank" rel="noreferrer">2. Get devnet USDC ↗</a></div></div><ol><li><span>01</span><div><strong>Use Phantom — no new wallet</strong><p>Open Phantom’s network selector and choose Solana Devnet.</p></div></li><li><span>02</span><div><strong>Fund the same public address</strong><p>Request both faucets using the address shown in Phantom. Never share its recovery phrase.</p></div></li><li><span>03</span><div><strong>Confirm your test USDC</strong><p>{!connected ? "Connect Phantom to check it here." : usdcBalance === null ? "Checking your devnet USDC balance…" : usdcBalance > 0 ? `${usdcBalance.toLocaleString()} test USDC detected.` : <>No devnet USDC detected yet. If Phantom does not display it, add mint <code>{DEVNET_USDC_MINT.toBase58()}</code>.</>}</p></div></li></ol></section>;
}
...
export default function LiveApp() {
  ...
  async function submitCreateRoom(event: React.FormEvent) {
    ...
    try {
      ...
      const result = await createCampaign(activeWallet, {
        ...
        depositCap: BigInt(Math.round(createForm.deposit * LAMPORTS_PER_SOL)),
        minGoal: BigInt(Math.round(createForm.minGoal * LAMPORTS_PER_SOL)),
        maxGoal: BigInt(Math.round(createForm.maxGoal * LAMPORTS_PER_SOL)),
        deadline,
      });
      setCreateProgress(2);
  ...
  async function submitCommitment() {
    ...
    try {
      ...
      const result = await createPrivateCommitment(
        activeWallet,
        BigInt(Math.round(maxBudget * LAMPORTS_PER_SOL)),
        1,
        selectedAddress,
        setCommitProgress,
      );
      ...
      setCommitState("done");
      setCommitNotice("Your equal deposit is public; your maximum is protected inside the Private ER.");
  ...
  async function submitHostQuote() {
    ...
    try {
      ...
      await claimInviteIfPresent(activeWallet);
      const signature = await postSupplierOffer(activeWallet, hostQuantity, BigInt(Math.round(hostPrice * LAMPORTS_PER_SOL)), selectedAddress);
  ...
  async function settleRoom() {
    ...
    setOrganizerNotice("");
    try {
      const result = await runOrganizerSettlement(wallet, selectedAddress, BigInt(Math.round(finalGoal * LAMPORTS_PER_SOL)), setOrganizerProgress);
      setOrganizerSignatures(result.signatures);
      setOrganizerState("done");
      setOrganizerNotice(result.finalStatus === "settled" ? "The group cleared. Supplier payment and participant refunds are ready." : "The group did not clear. Every participant can reclaim the full deposit.");
      await refreshRoom();
    } catch (error) {
  ...
  async function claim(kind: "refund" | "receipt") {
    ...
    try {
      ...
      setCommitNotice(kind === "refund" ? "Refund claimed on Solana." : "Prototype purchase receipt created on Solana.");
    } catch (error) {
  ...
  return <main className="product-app">
    ...
    {view === "room" && <div className="app-shell app-page room-page">{roomState === "loading" && <div className="room-loading"><span /><p>Reading room state, quotes and your position from Solana…</p></div>}{roomState === "error" && <div className="room-error"><HomeIcon size={28} /><h2>We couldn’t open this room</h2><p>{roomError}</p><button className="app-secondary" onClick={() => navigate("lobby")}>Return to your rooms</button></div>}{campaign && selectedMeta && roomState === "ready" && <>
      ...
      <section className="room-hero"><div className={`room-hero-art tone-${campaign.address.charCodeAt(0) % 3}`}><div className="room-hero-overlay"><span>{selectedMeta.propertyType}</span><strong>{selectedMeta.location}</strong></div><BuildingIcon size={105} /></div><div className="room-hero-copy"><div className="room-title-line"><div><span className={`room-status ${campaign.status}`}><i />{statusLabel(campaign.status)}</span><h1>{campaign.title}</h1><p>{selectedMeta.description}</p></div><button className="invite-button" onClick={() => void copyInvite("participant")}><CopyIcon size={15} />{inviteState === "copied" ? "Invite copied" : inviteState === "error" ? "Copy failed" : "Copy participant invite"}</button></div><div className="room-detail-row"><span><CalendarIcon /><small>DATES</small><strong>{selectedDates}</strong></span><span><UsersIcon /><small>GROUP</small><strong>{campaign.totalRequested} of {campaign.targetQuantity} committed</strong></span><span><WalletIcon /><small>EQUAL DEPOSIT</small><strong>SOL {displayAmount(campaign.depositCap)} per person</strong></span><span><KeyIcon /><small>DEADLINE</small><strong>{formatCountdown(campaign.deadline - now)}</strong></span></div></div></section>
      ...
      {(campaign.status === "settled" || campaign.status === "cancelled") && <section className={`outcome-banner ${campaign.status}`}><span>{campaign.status === "settled" ? <CheckIcon size={28} /> : <ArrowIcon size={28} />}</span><div><small>FINAL ROOM OUTCOME</small><h2>{campaign.status === "settled" ? "The group cleared one fair quote." : "The room did not clear. Nobody is stuck with the bill."}</h2><p>{campaign.status === "settled" ? `${campaign.allocatedQuantity} participants were allocated at SOL ${winningPrice} each with supplier ${winningOffer}.` : "All participant allocations are zero and the complete public deposits are refundable."}</p></div>{position && <div className="personal-outcome"><small>YOUR OUTCOME</small><strong>{position.allocation > 0 ? `${position.allocation} allocation` : "Not allocated"}</strong><span>Refund: SOL {displayAmount(position.refundOwed)}</span></div>}</section>}

      <section className="role-switcher"><div><span className="app-kicker">CHOOSE YOUR VIEW</span><h2>One room. Three distinct jobs.</h2></div><div role="tablist" aria-label="Room role"><button className={role === "participant" ? "active" : ""} disabled={role !== "participant"} onClick={() => setRole("participant")} role="tab" aria-selected={role === "participant"}><UsersIcon /><span><strong>Participant</strong><small>Set a private limit</small></span></button><button className={role === "host" ? "active" : ""} disabled={role !== "host"} onClick={() => setRole("host")} role="tab" aria-selected={role === "host"}><BuildingIcon /><span><strong>Deal publisher</strong><small>Quote the whole group</small></span></button><button className={role === "organizer" ? "active" : ""} disabled={role !== "organizer"} onClick={() => setRole("organizer")} role="tab" aria-selected={role === "organizer"}><ShieldIcon /><span><strong>Organizer</strong><small>Close, match and settle</small></span></button></div></section>
