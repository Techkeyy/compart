...
export default function LiveApp() {
  ...
  async function settleRoom() {
    ...
    setOrganizerNotice("");
    try {
      const result = await runOrganizerSettlement(wallet, selectedAddress, setOrganizerProgress);
  ...
  return <main className="product-app">
    ...
    {view === "lobby" && <div className="app-shell app-page lobby-page">
      ...
      <section className="entry-grid"><article className="entry-card create-entry"><span className="entry-icon"><PlusIcon /></span><small>FOR ORGANIZERS</small><h2>Start a purchase room</h2><p>Choose the group size, dates, equal deposit, deadline and public terms. Compart creates an unlisted room and gives you invitations to share.</p><button onClick={() => navigate("create")}>Configure room <ArrowIcon size={16} /></button></article><article className="entry-card join-entry"><span className="entry-icon"><KeyIcon /></span><small>HAVE AN INVITATION?</small><h2>Open your private room</h2><p>Paste the complete participant or deal-option invitation you received. We verify the underlying room before opening it.</p><label><span>Invitation link</span><div><input id="join-room" value={joinCode} onChange={(event) => setJoinCode(event.target.value)} placeholder="Paste invitation link" /><button onClick={joinRoom} disabled={joinState === "working"}>{joinState === "working" ? "Checking…" : "Open"}</button></div></label>{joinNotice && <p className="entry-notice error">{joinNotice}</p>}</article></section>

      <section className="getting-started"><div><span className="app-kicker">FIRST TIME ON DEVNET?</span><h2>Start in three small steps.</h2><p>Compart uses free devnet SOL for prototype transactions. It is not real money and does not book real accommodation.</p><a className="faucet-link" href="https://faucet.solana.com" target="_blank" rel="noreferrer">Get free devnet SOL ↗</a></div><ol><li><span>01</span><div><strong>Connect your wallet</strong><p>Use Phantom or Solflare and switch it to Solana Devnet.</p></div></li><li><span>02</span><div><strong>Create or open a room</strong><p>Organizers create a room. Invitees open the one-time link they received.</p></div></li><li><span>03</span><div><strong>Claim your role, then act</strong><p>Your first commitment or quote claims the link to your connected wallet.</p></div></li></ol></section>
