...
export default function LiveApp() {
  ...
  async function submitCreateRoom(event: React.FormEvent) {
    ...
    try {
      ...
      const result = await createCampaign(activeWallet, {
        ...
        depositCap: BigInt(Math.round(createForm.deposit * 10_000)),
        deadline,
      });
      setCreateProgress(2);
  ...
  async function submitHostQuote() {
    ...
    try {
      ...
      const signature = await postSupplierOffer(activeWallet, hostQuantity, BigInt(Math.round(hostPrice * 10_000)), selectedAddress);
