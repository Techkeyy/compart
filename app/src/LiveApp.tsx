...
export default function LiveApp() {
  ...
  async function cancelRoomForFullRefund() {
    if (!selectedAddress || !wallet?.publicKey) return;
    setOrganizerState("working");
    setOrganizerNotice("");
    try {
      const signatures = await cancelStalledCampaign(wallet, selectedAddress);
      setOrganizerSignatures(signatures);
      setOrganizerState("done");
      setOrganizerNotice("Room cancelled. Full USDC refunds were sent to every participant.");
      await refreshRoom();
    } catch (error) {
      setOrganizerState("error");
      setOrganizerNotice(error instanceof Error ? error.message : "Cancellation could not complete.");
    }
  }
